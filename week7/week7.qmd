...
**RESPONSE:** You cannot compare another state because each state differs in environmental policy, economic growth trajectories, population, etc. They are not strong counterfactuals.
...
**RESPONSE:** You want the trends for the synthetic control to track closely.
...
**RESPONSE:** We include the outcome variable as a predictor because it can help identify similar/more accurate synthetic. 
...
**RESPONSE:** 49 states in the donor pool and 23 years. 
...
scm_data <- data %>%
  #
  synthetic_control(
    outcome = co2_power_pc_tons, # Outcome variable: per capita CO2
    unit = state, # Unit identifier (state)
    time = year, # Time identifier (year)
    i_unit = "CA", # Intervention (treated) unit
    i_time = 2013, # Intervention (treatment) year: 2013 
    
    # Generate placebo tests for all states 
  ...
  # every state has 4 numers that estimate its pre-treatment characteristics 
  ...
  generate_weights(
    ...
    bound_ipop = 6) %>%
  # construct synthetic California using optimized donor weights 
  generate_control() 
```
...
**RESPONSE:** The synthetic and observed follow each other before 2013, although California is more variable. The quality of the synthetic control looks good so far. 

2.  What happens to the gap between the two lines after 2013? What does this suggest about the effect of cap-and-trade?

**RESPONSE:** The gap between the two lines after 2013 signifies that the cap and trade policy was effective. 
...
scm_data %>% 
  plot_differences() +
  labs(title = "Gap: Obseerved - Synthetic")
...
**RESPONSE:** You expect the pre-treatment to hover around 0 and the post-treatment to show a big effect (changing from 0 to 2).  
...
**RESPONSE:** We want to see if CA stands out against all the placebos. You see a big drop for the California. 
...
$$
- ratio would be close to 1 if there is no different after treatment
- larger ratio means bigger effect post-treatment
...
**RESPONSE:** California is the first in the MSPE ratio distribution. This implies that there was a big effect post-treatment. 
...
**RESPONSE:** Synthetic California matches well with real California on each predictor except for elec_price. 

2.  How does the synthetic control compare to the simple donor pool average?

**RESPONSE:** Synthetic control and real are more close than the donor sample. 
...
**RESPONSE:** California cap and trade program was effective. The trends plot, gap plot, and placebo tests all showed difference sin co2 post treatment. 
...
**RESPONSE:** The trends plot showed that there was a strong pre-treatment fit. The donor pool was not chosen transparently in this case since we included treated states. Spillover is a possibility. Placebo tests showed that California showed a difference in co2 against states that did not have a cap and trade aka the control units. 
...
**RESPONSE:**  It is important in order to see the full effect of the policy on California. 
...
**RESPONSE:** SCM is different from matching because the counterfactual is a synthetic 
