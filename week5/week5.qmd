...
**RESPONSE**: The two lines represent control area (0) and protected area (1), their trends reveal that deforestation rates decrease drastically as they near the boundary to protected areas. The negative values represent outside the boundary of protected areas and inside represent the positive values. If there was no discontunity, then protected areas would not be as effective for protecting forested lands. 
...
rdd_ols <- lm(
  deforest ~ # outcome
    ...
    protected * distance + # allows slopes to vary
    slope_cat + road_cat + water_cat + soil_cat,  # controls/covariates 
    data = sim_data
)
...
**RESPONSE**: The estimated causal effect of protected area status is a 10% decrease in deforestation, this is similar to the linear regression plot. The interaction coefficient represents the slope of the protected areas, this would be .0002 higher than unprotected areas.
...
**RESPONSE**: The smaller the bandwidth, the lower the bias and higher variance. The larger the bandwidth, the higher the bias and lower variance. Due to this, the cutoff reduces the dataset and makes the ATE smaller. Another reason is that we are weighing points closer to the boundary, which puts more emphasis on those points in relation to distance. 
...
**RESPONSE**: The rdd plot improves your predictions. The linear model shows a more linear relationship, while the rdd plot predicts witht the data better. 
...
**RESPONSE**: Although the running distance differs between our plot, the plot we created and the the Neal 2024 plot are very similar in deforestation rates. 
