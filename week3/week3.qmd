...
weekly_la_phx <- did_data_az_ca %>% 
  mutate(
    # change order of datetime 
    date_obj = mdy(date), 
    # Assigning a week for each observation 
    week = floor_date(date_obj, unit = "week"),
  ) %>% 
  # filter to year 2020 and months 2,3, 4
...
This violates the parallel trends assumption because of the spike in NO2 pre-lockcdown for LA. This is not a good apples-to-apples comparison. 
...
These cities are more similar and thefore have an apples ot apples comparison. 
...
```{r}
# avg NO2 for obs in NM after lockdown
y_trt_post <- did_data_nm_tx %>% filter(treated == 1, is_post  == 1, year == 2020, 
 ...
 # pull out the value itself 

# avg NO2 for obs in NM pre lockdown 
y_trt_pre <- did_data_nm_tx %>% filter(treated == 1, is_post  == 0, year == 2020,
                                       month(date_obj) %in% c(2,3,4)) %>%
  summarise(mean(daily_max_1_hour_no2_concentration)) %>% pull()

y_con_post <- did_data_nm_tx %>% filter(treated == 1, is_post  == 1, year == 2020,
                                        month(date_obj) %in% c(2,3,4)) %>%
  summarise(mean(daily_max_1_hour_no2_concentration)) %>% pull()
...
trt_effect <- (y_trt_post- y_trt_pre) - (y_con_post - y_con_pre)# pull out the value itself
...
- look at per year NO2 
- look at the population between counties 
...
- the treat:is_post coefficient is the treatment effect; this is not significant signifying that the relationship between lockdown and N0_2 is not strong enough for these two cities 
...
A dust storm would not ruin the DiD framework of the study because the two counties are good apples-to-apples and if a single dust storm affects one, it affects both. El Paso TX was the counterfactual. 
...
The treatment effect would be smaller because the differences between treatment and control groups would become smaller. 
