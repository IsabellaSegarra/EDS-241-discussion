...
class_data <- read_csv("https://raw.github.com/garberadamc/W26-Policy-Eval/main/course-materials/labs/data/W26_class_survey.csv") %>% 
  # mutate to add a oclumn that randomly places students in treatment and control
...
tbl_2way <- lobsters %>% 
  ...
  summarize(
    total_cpue = sum(cpue, na.rom = TRUE), 
    .groups = "drop" # same as ungrouping 
  ) %>% 
  # Pivto to create a column for each region
  pivot_wider(
    names_from = region, 
...
tbl_3way %>% 
  ...
  tab_header(
    title = "European Lobster Catch by Region, Year, and Treatment Group", 
...
table_w_plots <- lobsters %>% 
  ...
  # Create gt table 
  gt() %>% 
  tab_header(title = "European Lobster Catch Totals and Distirbution (2006-2010)",
  subtitle = "Total Catch per Unit Effort (CPUE)") %>% 
  cols_label(
    year = "Year", 
...
library(cowsay)
library(beepr)
say("All done making some beautiful tables! :) ", "owl"); beep(3)  
