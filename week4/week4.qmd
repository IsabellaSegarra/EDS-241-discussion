...
*Response:* The pollution risk, reef type, reef slope covariates are different between MPA and non-MPA. 
...
*Response:* A large difference in certain between the two sites can signify a confounding relationship. This can confound the relationship of MPAs on fish biomass because it reduces the apple-to-apples comparsion, for instance, there are naturally smaller fish in mangroves t `dist_mangrove` and thus comparing fish biomass between the habitats will not show the relaitonship as clearly. 
...
*Response:* Average treatment of the treated means the average fish biomass in our treated or MPA sites. 
...
*Response:* 108 matched treatments in the treated group and 37 matched treatments in the control sites. 

**Q6.** Find the covariate with the largest standardized mean difference (SMD) *after matching*. Which covariate is it?

*Response:* Before and after matching, the largest SMD was `reef_exposureExposed`. Why did it change? By  optimizing balance on some controls, it changes the balance on other controls. 
...
reg_ecological <- lm(biomass_ecological ~ treated_mpa, 
                     data = matched_data, 
                     weights = weights) # some control sites were matched multiple times and recieve larger weights 
...
*Response:* For the fishers coefficient , the OLS model predicts that treated MPA sites will have a larger average treated of the treated than control sites. 

**Q8.** What is the biggest remaining threat to causal interpretation of regression coefficients after matching?

*Response:* The biggest threat to causal interpretation of regression coefficients after matching is other confounding variables not included in the model. 
...
*Response:* `vpd` coefficient increased between the no fixed effect models and fixed effect models. When controlling for plot differences, you can see that the coefficient increases and therefore the plot variables have an influence on `vpd`. 
...
*Response:* Year is an important fixed effect to include in this study because some years could be associated with more rainfall and influence `vpd`. 
...
*Response:* Plot is important to include as a fixed effect because there can be variations in the tree dbh between plots because the characteristics of the site. It is important to contorl for this because it influences the `vpd` coefficient. 
