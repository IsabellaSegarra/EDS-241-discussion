...
**RESPONSE:** Some potential source of endogeneity could arise to a surge in pries of inhalers, which would cause an increase in inhalers search. Weather and temperature can influence PM 2.5 and google trends. Wildfire events can influence google searches for asthma inhaler as well as influence weekly total precipitation. 
...
```{r}
options(noaakey = "iSSbFQSivWezVqubTFTjqABuhNzXvzTX
 	")
```
...
trends <- gtrends(
  keyword = "asthma inhaler",  #search word
  geo = "US-CA-803", # LA specific code
  time = "2023-01-01 2023-12-31",
  onlyInterest = TRUE # only keeps over the time series you have 
)
# select df, clean df for interested time 
trends_df <- trends$interest_over_time %>% 
  ...
  filter(!is.na(asthma_index)) %>% # filter out Nas
  select(date, asthma_index) # asthma_index is a scale from 0 to 100 (max hits)
```
...
**RESPONSE:** The coefficient changed from a p-value of 0.5 to 0.7. Seasonal confounders are acting on pm25, however bias still remains for weather patterns. 
...
first_stag_summary <- summary(first_stage)

first_stag_summary$fs[1]
...
**RESPONSE:** 1) precip total has a negative affecton pm2.5. 2) There is alot o f unexplained variance. 

F= variance explained by precip (IV)/ remained unexplained variance 
- smaller f stat: more unexplained variation
- larger f stat: less unexplained variation 
...
**RESPONSE:** People are less likely to go outside on rainy days, making it more likely to stay inside and search for inhalers. 
...
**RESPONSE:** To strengthen the role of precipitation, you can use a larger dataset (multiple cities, multiple years). 
...
**RESPONSE:** Another variable that might serve as an instrument for PM2.5 could be wind. 
...
**RESPONSE:** Four key assumptions to identify a causal effect.
1. relevance 
2. independence/exogeneity, the instrument needs to be random. Rainfall should not be correlated with unobserved drivers. 
3. There are no weeks where rain is increases pm25, something that would break it would be a non-constant trend. 
