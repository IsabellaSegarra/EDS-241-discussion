...
```
Intercept (77.91): Without any treatment, we expect the number of treat piles to be ~78 on average. 

Treatment (-24.77): The treatment reduced the average number of trash piles by approximately 25 piles. 
...
```
Interpretation:
Intercept (78.25): On average, the model estimates our control group to have 78 waste piles. 
Treatment (exp^-0.38-1 * 100) -32: The model estimates that the treatment reduced waste burning by 32% in treated neighborhoods. 

Similar intercept values to OLS. 
...
```
Interpretation:
Intercept (74.44): On average, the model estimates our control group to have 74 waste piles.
Treat(-34.3): The model estimates that the treatment reduced waste burning by 34.3% in treated neighborhoods. 
...
```
Interpretation:
Intercept(1.59): Our model estimates the waste piles for the control region, when controlling for the pre count average, is a 1.6 average. 
treat (-23.6): There is a 23.6% reduction in waste piles in the treat groups when controlling for the pre-count average. 
log(pre_count_avg)=-0.15
